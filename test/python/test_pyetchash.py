...
def test_get_cache_size_not_None():
    for _ in range(100):
        ...
        out = pyetchash.get_cache_size(block_num)
        assert out != None

def test_get_full_size_not_None():
    for _ in range(100):
        block_num = randint(0,12456789)
        out = pyetchash.get_full_size(block_num)
        assert out != None

def test_get_cache_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pyetchash.get_cache_size(block_num)
        out2 = pyetchash.get_cache_size((block_num // pyetchash.EPOCH_LENGTH) * pyetchash.EPOCH_LENGTH)
        assert out1 == out2

def test_get_full_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pyetchash.get_full_size(block_num)
        out2 = pyetchash.get_full_size((block_num // pyetchash.EPOCH_LENGTH) * pyetchash.EPOCH_LENGTH)
        assert out1 == out2

# See light_and_full_client_checks in test.cpp
def test_mkcache_is_as_expected():
    actual = pyetchash.mkcache_bytes(
        1024,
        "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~").encode('hex')
...
def test_calc_dataset_is_not_None():
    cache = pyetchash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    assert pyetchash.calc_dataset_bytes(1024 * 32, cache) != None
...
def test_light_and_full_agree():
    ...
    full_size = 1024 * 32
    header = "~~~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~"
    light_result = pyetchash.hashimoto_light(full_size, cache, header, 0)
    dataset = pyetchash.calc_dataset_bytes(full_size, cache)
    full_result = pyetchash.hashimoto_full(dataset, header, 0)
...
def test_mining_basic():
    ...
    assert type(pyetchash.mine(dataset,header,easy_difficulty)) == dict
    assert type(pyetchash.mine(dataset,header,easy_difficulty)["nonce"]) == long
    assert type(pyetchash.mine(dataset,header,easy_difficulty)["mix digest"]) == str
    assert type(pyetchash.mine(dataset,header,easy_difficulty)["result"]) == str
...
def test_mining_doesnt_always_return_the_same_value():
    ...
    dataset = pyetchash.calc_dataset_bytes(full_size, cache)
    # Check type of outputs
    assert pyetchash.mine(dataset, header, easy_difficulty1)['nonce'] != pyetchash.mine(dataset, header, easy_difficulty2)['nonce']

def test_get_seedhash():
    assert pyetchash.get_seedhash(0).encode('hex') == '0' * 64
    import hashlib, sha3
    expected = pyetchash.get_seedhash(0)
    #print "checking seed hashes:",
    for i in range(0, 30000*2048, 30000):
        #print i // 30000,
        assert pyetchash.get_seedhash(i) == expected
